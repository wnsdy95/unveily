...
- The bounded last selected web-analysis mode (`page` or `cookies`); pasted text and analysis results are not stored with this preference.
...
The popup does not write the analysis-mode preference directly. It sends an exact, bounded `page` or `cookies` request to the trusted service worker, which serializes writes and makes preference reads wait for all earlier writes. If trusted local-storage isolation is unavailable, the next popup defaults to current-page analysis without reading or writing local state. If an individual preference write fails after isolation has succeeded, the last successfully saved mode remains in effect. Current-page, cookie, and pasted-text analysis remain usable even when the preference cannot be persisted.
