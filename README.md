...
- Remembers the last selected current-page or cookie analysis mode across navigation, tab changes, popup reopen, and browser-worker restarts. The service worker serializes mode changes and makes a later read wait for earlier writes, so closing the popup does not leave the last click queued only in that popup. Paste analysis does not replace that saved web-analysis mode.
...
`npm test` is the fast Node suite with Chrome API mocks. `npm run test:e2e:chrome` launches a real Chrome 140+ installation, loads the unpacked extension in a clean temporary profile, and uses controlled localhost fixtures to verify the companion-overlay path, including explicit enablement, hidden-tab DOM idling/resume while value-free request observation continues, popup analysis and scrolling, page/cookie mode persistence across popup teardown, navigation, and a Manifest V3 worker restart, continuous colors, stale-document rejection, and document isolation. Set `E2E_CHROME_PATH` when Chrome is installed outside the detected platform paths; CI runs the same command headlessly.

### Optional manual public-site smoke

`npm run test:live-sites -- --live --all` is a manual-only, opt-in smoke check against the repository's fixed list of public HTTPS pages. Both the explicit `--live` flag and exactly one scope (`--all` or `--only=<target-id,...>`) are required; otherwise the command fails closed. It also refuses to run when `CI` or `GITHUB_ACTIONS` is set, and it must never be added to CI, release gates, or the routine command block above. Public pages change independently of this project, so run the check sparingly, follow each site's terms, and treat blocked or changed pages as observations rather than deterministic product failures. Use `--headed` to watch the run or run `npm run test:live-sites -- --live --only=naver-privacy,example-ordinary` to limit it to named targets.

Run `npm run test:live-sites:safety` first. It contacts only a temporary `127.0.0.1` fixture and behaviorally checks the harness's GET interceptor plus its service-worker, worker, beacon, event-stream, WebSocket, scripted-form, new-target, and cleanup guards. A live run stops if that preflight fails.

The smoke check visits one unauthenticated page at a time and performs no public-site clicks, scrolling, text input, form submission, login, purchase, or consent choice. On the instrumented public page and its frames, non-GET and non-HTTPS HTTP requests, streaming transports, IP literals, single-label or common special-use/local hosts, and unexpected top-level origins are blocked. Downloads are denied; browser-level WebSocket/FTP/file URL blocking is also enabled. Before site script runs, the harness disables service-worker registration, dedicated/shared workers, WebSocket, EventSource, WebTransport, WebRTC, beacon, popups, scripted form submission, and scripted link/click activation. This deliberately reduced runtime can break hydration or other site behavior, which is reported as `inconclusive` rather than an extension failure. These controls are defense in depth around the Puppeteer-managed page, not an OS-level network sandbox: Chrome features outside that target boundary and public DNS names that resolve to private addresses are not independently proven safe. Normal HTTPS third-party GET subresources can still load, and servers can log requests or set cookies.

One clean temporary profile is shared by the selected targets to test navigation persistence, so cookies, cache, and third-party state can carry between targets inside that run. Cleanup is attempted after every run and every failure is recorded in `cleanupErrors`; if that array is non-empty, identify that run's `unveily-public-smoke-*` directory under the OS temporary directory before removing only that profile. Every target must retain an exact reviewed final-document fingerprint (origin, path, and query; fragments are ignored because they do not select a different HTTP document) and a target-specific in-memory content marker before extension assertions count; changed, redirected, bot-challenge, or partially rendered pages are `inconclusive`. The two-target-or-larger scope also verifies page→cookie mode persistence on the next page, immediate-close page restoration, popup reopening, and worker restart.

The script writes only a coarse local report to `reports/public-site-smoke/*.local.json`, which Git ignores. It stores target IDs, origins, booleans, counters, and `pass`/`warn`/`inconclusive`/`fail` outcomes—not paths, query strings, raw page text, cookie values, or raw request URLs. Review the local report before sharing it and delete it locally when it is no longer needed.
...
- `src/analysisModePreference.js`: Bounds and persists the last selected current-page or cookie analysis mode behind trusted local-storage isolation.
