...
- Page/cookie mode persistence accepts only exact messages from this extension's own `chrome-extension://` pages. The service worker owns one serialized write queue, validates the two allowed modes, and delays GET responses behind preceding writes so rapid clicks and popup teardown cannot reorder the committed preference. Storage-isolation failure returns the default page mode without touching local state; an individual write failure retains the last successfully committed mode. Neither failure blocks value-free analysis.
